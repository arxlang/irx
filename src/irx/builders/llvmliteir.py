...
class LLVMLiteIRVisitor(BuilderVisitor):
    ...
    @dispatch  # type: ignore[no-redef]
    def visit(self, node: irx_arrow.ArrowFloat32Array) -> None:
        """
        title: Lower the internal Arrow float32 array helper.
        parameters:
          node:
            type: irx_arrow.ArrowFloat32Array
        """
        builder_new = self.require_runtime_symbol(
            "arrow", "irx_arrow_array_builder_float32_new"
        )
        append_float32 = self.require_runtime_symbol(
            "arrow", "irx_arrow_array_builder_append_float32"
        )
        finish_builder = self.require_runtime_symbol(
            "arrow", "irx_arrow_array_builder_finish"
        )
        ...
        builder_slot = self._llvm.ir_builder.alloca(
            self._llvm.ARROW_ARRAY_BUILDER_HANDLE_TYPE,
            name="arrow_f32_builder_slot",
        )
        self._llvm.ir_builder.call(builder_new, [builder_slot])
        builder_handle = self._llvm.ir_builder.load(
            builder_slot, "arrow_f32_builder"
        )
        ...
        for item in node.values:
            ...
            value = safe_pop(self.result_stack)
            if value is None:
                raise Exception("Arrow float32 helper expected a float value")
            if not is_fp_type(value.type):
                raise Exception(
                    "Arrow float32 helper supports only float expressions"
                )

            self._llvm.ir_builder.call(append_float32, [builder_handle, value])

        array_slot = self._llvm.ir_builder.alloca(
            self._llvm.ARROW_ARRAY_HANDLE_TYPE,
            name="arrow_f32_array_slot",
        )
        self._llvm.ir_builder.call(
            finish_builder, [builder_handle, array_slot]
        )
        array_handle = self._llvm.ir_builder.load(
            array_slot, "arrow_f32_array"
        )
        length_i64 = self._llvm.ir_builder.call(
            array_length, [array_handle], "arrow_f32_length"
        )
        self._llvm.ir_builder.call(release_array, [array_handle])

        length_i32 = self._llvm.ir_builder.trunc(
            length_i64, self._llvm.INT32_TYPE, "arrow_f32_length_i32"
        )
        self.result_stack.append(length_i32)
